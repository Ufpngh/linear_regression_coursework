...
2. Загрузить все файлы проекта в корень репозитория.
3. Открыть **Settings → Pages**.
4. В разделе **Build and deployment** выбрать ветку `main`.
5. В качестве папки выбрать `/root`.
6. Сохранить настройки.
7. После публикации GitHub Pages откроет `index.html` из корня проекта.
...
2. Объяснить, что регрессия предсказывает число.
3. Показать формулу `ŷ = a·x + b`.
4. Объяснить residual: `eᵢ = yᵢ - ŷᵢ`.
5. Объяснить RSS и почему ошибки возводятся в квадрат.
6. Показать раздел, где объясняется `a = Sxy / Sxx`.
7. Показать, что `b = ȳ - a·x̄`, поэтому лучшая линия проходит через центр данных.
8. Объяснить R² через baseline “всегда среднее y”.
9. Открыть `app.html`.
10. Добавить точки на график.
11. Перетащить точку и показать, что линия, residuals и метрики меняются сразу.
12. Выбрать точку и показать `x`, `y`, `ŷ`, residual и `residual²`.
13. Удалить точку кнопкой или клавишей Delete.
14. Переключить модель с линии на параболу.
15. Переключить учебные наборы: простой, шумный, с выбросом, нелинейный.
16. Сгенерировать линейные точки, параболические точки и выброс.
17. Показать сравнение MSE и R² для линии и параболы.
18. Открыть `notebooks/linear_regression_demo.ipynb`.
19. Показать ручной расчёт коэффициентов и проверку через `sklearn`.
20. Показать реалистичный учебный датасет и объяснить, почему одной площади мало для оценки цены.
21. Открыть `docs/CODE_WALKTHROUGH.md`, если преподаватель попросит объяснить код.
